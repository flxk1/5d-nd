...
#
# ─────────────────────────────────────────────────────────────────────────────
# VENDORED from loomground-solver (module ``loomground_solver.dimensions``).
#
# The authoritative dimension vocabulary + composition table lives THERE, not
# here. This is a faithful, unmodified copy so the thin ``5d+nd`` grounding
# resolver can canonicalize and validate a reference's dimensions WITHOUT taking
# a hard runtime dependency on the solver. It owns nothing — it re-exports one
# module of an upstream package.
#
# SYNC: the authoritative table lives in loomground-solver, not here. Install the
#       ``solver`` extra (``pip install '5d-nd[solver]'``) to depend on the real
